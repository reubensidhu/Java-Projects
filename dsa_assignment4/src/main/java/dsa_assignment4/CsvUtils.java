...
public class CsvUtils
{
			...
			while (!(c)) {
				try ( PrintWriter to = new PrintWriter(tmpPath.toFile()))
				{
					for (int i = 0; i<numRowLimit; i++) {
 						if ((row = formatter.readRow(from)) != null) {
							pq.add(row);
						}
 						else {
 							c = true;
					...
					tempNum+=1;
					tmpPath = fromPath.resolveSibling(
							String.format("temp_%05d_%s", tempNum, fromPath.getFileName())) ;
		...
		throws Exception
	{
		...
		{
			CsvFormatter formatter = new CsvFormatter(from1);
			...
			try ( PrintWriter to = new PrintWriter(outputPath.toFile()))
			{
				...
				String[] row3;
				String[] row4;
				row3 = formatter.readRow(from1);
				row4 = formatter2.readRow(from2);
				boolean empty = false;
				while(empty==false) {
					if (row3 == null) {
						while ((row4  != null)) {
							formatter2.writeRow(to, row4);
							row4 = formatter2.readRow(from2);
						}
						empty = true;
					}
					if (row4 == null) {
						while ((row3) != null) {
							formatter.writeRow(to, row3);
							row3 = formatter.readRow(from1);
						}
						empty = true;
					}
					
					else if (comparator.compare(row3, row4)<0) {
						formatter.writeRow(to, row3);
						row3 = formatter.readRow(from1);
					}
					else if (comparator.compare(row3, row4)>0) {
		...
		throws Exception
	{
		...
		while (paths.size()>2) {
				Path tmpPath = outputPath.resolveSibling(
						String.format("temp_%05d_%s", tempNum, outputPath.getFileName()));
				mergePairCsv(paths.poll(), paths.poll(), columnName, tmpPath);
				paths.offerLast(tmpPath);
				tempNum += 1;
		}
		if (paths.size()==2){
			mergePairCsv(paths.poll(), paths.poll(), columnName, outputPath);
			paths.offerFirst(outputPath);
			}
		return true;
	}
}
