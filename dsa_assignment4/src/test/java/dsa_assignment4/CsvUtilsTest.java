...
public class CsvUtilsTest
{
		...
		throws Exception
	{
		...
		// Now check that the two files are the same
	...
	/**
	 *  Test that, when trying to read from a bad csv file (in this case one which has a row that has one too many fields), that an exception is thrown.
	 * @throws Exception
	 */
		...
		throws Exception
	{
		...
		// The combined file is not normally ordered so don't check that
		...
		throws Exception
	{
		...
		// Since we are merging, we don't care how many rows the split files have,
		// so numRowLimit = 0, meaning it won't be checked
		// The combined file should be ordered so check it
		// This can only work if the split files are ordered, so check that too - even
		// though such an error would not be an error in mergePairCsv
		int numRowLimit = 0;
	...
	@Test
	public void mergePairCsvTest2()
		throws Exception
	{
		Path split1Path = dataDir.resolve("split_2_state-abbrevs.csv");
		Path split2Path = dataDir.resolve("split_3_state-abbrevs.csv");
		Path combinedPath = dataDir.resolve("temp_merged_2_3_state-abbrevs.csv");
		String columnHeader = "abbreviation";
	...
	@Test
	public void mergePairCsvTest3()
		throws Exception
	{
		Path split1Path = dataDir.resolve("split_4_state-abbrevs.csv");
		Path split2Path = dataDir.resolve("split_5_state-abbrevs.csv");
		Path combinedPath = dataDir.resolve("temp_merged_4_5_state-abbrevs.csv");
		String columnHeader = "abbreviation";
	...
	//@Test
	public void mergeListCsvTest3()
		throws Exception
	{
		Path[] splitPaths = { //
				dataDir.resolve("split_0_state-abbrevs.csv") }; //
				 
		Path combinedPath = dataDir.resolve("temp_merged_single_state-abbrevs.csv");
		String columnHeader = "abbreviation";
	...
	/**
	 * This method, if enabled, removes all temporary files from the data directory after running the tests.
	 * To enable it, uncomment the "@AfterClass" annotation and comment out the "@Ignore" annotation. 
	 */
	@Ignore
	//@AfterClass
	public static void cleanUp()
	{
		...
		{
			logger.error(String.format("Data directory \"%s\" is not a directory!", dataDirFile.getName()));
			return;
		}
		...
		for (File file : dataDir.toFile().listFiles(filenameFilter))
		{
